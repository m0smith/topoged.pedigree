(ns topoged.pedigree.ui
  (:require [om.core :as om :include-macros true]
            ...
            [topoged.pedigree.core :refer [gedcom-file parents-of]]))
...
(defn  open-gedcom [f]
  ...
  (let [file (aget (.. f -target -files) 0)
     ...
     (set! (.-onload reader) 
           #(.log js/console (first (gedcom-file (.. % -target -result)))))
  ...
  (fn [app owner]
    (om/component 
      (apply dom/select #js {:onChange jj}
        (map  (fn [[text value]] (dom/option #js {:value value :key value} text)) (:names app)))))
  app-state
